...
##########################################
def catchup():
    loginurl   = userinfo.host + ':' + userinfo.port + '?username=' + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def addonsettings(url,description):
	if   url =="CC":
		...
		dialog = xbmcgui.Dialog().yesno('TV Guide','Would You Like To Setup The PVR TV Guide?')
		if dialog:
			pvrsetup()
